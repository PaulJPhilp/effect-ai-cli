...
export const effectPatternsList = makeCommand(
  "list",
  { pathspec, verbose, json, quiet, output, force },
  ({ pathspec, verbose, json, quiet, output, force }) => {
    return Effect.gen(function* () {
      const metrics = yield* MetricsService;
      const otel = yield* OtelService;

      const targetPath = pathspec.length === 0 ? "." : pathspec[0];

      const verboseMode = Option.getOrElse(
        verbose as Option.Option<boolean>,
        () => false
      );
